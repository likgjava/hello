...
(function($) {
	$.msgbox = function(opts) {
		...
		this.closeWrap = $('<div />').css({  // it is a, not div
			'text-align': 'right'
		}).append('<a href="javascript:;" style="color:#CCCCCC;">×</a>').appendTo(this.headWrap).find('a').click(function() {
			_this.close();
		...
		domready: function(){
			...
			switch(this.opts.type){
				case 'alert':
					...
					var closebutton = $('<input class="input" type="button" value=" OK " />').click(function(){
						_this.close();	
					});
